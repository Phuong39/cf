...
func init() {
	...
	cvmLsCmd.PersistentFlags().BoolVar(&cvmFlushCache, "flushCache", false, "刷新缓存，不使用缓存数据 (Refresh the cache without using cached data)")
	...
	cvmExecCmd.Flags().BoolVarP(&metaDataSTSToken, "metaDataSTSToken", "m", false, "一键获取实例元数据中的临时访问凭证 (Get the STS Token in the instance metadata)")
...
var cvmLsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		tencentcvm.PrintInstancesList(cvmLsRegion, running, cvmLsSpecifiedInstanceID, cvmFlushCache)
	},
}
...
var cvmExecCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if lhost != "" && lport == "" {
			...
			tencentcvm.CVMExec(command, commandFile, scriptType, cvmExecSpecifiedInstanceID, cvmExecRegion, batchCommand, userData, metaDataSTSToken, cvmFlushCache, lhost, lport, timeOut)
		}
	},
}
